...
export default function UpdateProduct() {
  ...
  useEffect(() => {
    try {
      const fetchProduct = async () => {
        const res = await fetch(`${apiUrl}/api/product/getproducts?productId=${productId}`)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/api/product/updateproduct/${id}/${currentUser._id}`, {
        method: "PUT",
        headers: {
    ...
    <div className="p-5 max-w-3xl mx-auto min-h-screen mt-32 border">
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col gap-4">
          ...
          {imageUrl && (
            <img src={imageUrl || "/placeholder.svg"} alt="Uploaded preview" className="w-[70%] h-72 justify-center" />
          )}
        </div>
